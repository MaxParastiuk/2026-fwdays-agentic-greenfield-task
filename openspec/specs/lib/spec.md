...
Framework-free shared Zod schemas, inferred TypeScript types, and validation helpers
used by agents, pipeline, API routes, and client forms. Single source of truth for
`CheckResult`, `PipelineResult`, and API request/response shapes (capability
`01-foundation`).
