...
Server-side Checker agent that scores a cover letter against the CV and job posting,
returning structured `CheckResult` (score, gaps, rationale) for the pipeline loop
(capability `03-checker-agent`).
