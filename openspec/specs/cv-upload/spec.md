...
Accept a visitor CV as PDF upload or pasted plain text, extract text server-side, validate inputs, and show a collapsed preview in the app shell CV panel.
...
### Requirement: Server-side file size limit

Uploaded files SHALL be rejected server-side at 5 MB (`CV_FILE_MAX_BYTES`) before the request body is buffered for parsing.

#### Scenario: Oversize file rejected server-side

- **WHEN** `POST /api/cv/parse` receives a multipart file larger than 5 MB
- **THEN** the handler returns HTTP 413 with an error message and does not invoke `pdf-parse`
